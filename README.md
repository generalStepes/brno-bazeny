...
- STAREZ venues' reservation grids can go stale during a long-term closure (Ponávka's did — some future days kept showing open slots during a closure running through 31 Dec 2026). The scraper also checks each venue's homepage alert banner for closure wording ("uzavřen"/"mimo provoz"/"odstávka") and forces every slot closed through the announced date range when found (both start and end, so a closure that hasn't started yet - e.g. Aquapark's, beginning 17 Aug - doesn't get applied prematurely), regardless of what the grid itself says. Benign banners (e.g. holiday hours notices) are left alone. Trimmed to just the closure sentence (unrelated logistics, like Ponávka's chip-return instructions, are dropped) and surfaced as a banner at the top of the page, not just buried in that venue's card.
